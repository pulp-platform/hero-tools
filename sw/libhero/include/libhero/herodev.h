...
typedef enum {
    ...
    ref_svm_mh = 0x13,     // pass by reference, SVM, do not set up mapping, use
                           // miss handling
    ref_copy_tryx = 0x14,  // pass by reference, no SVM, use contiguous L3
                           // memory, but to the tryx() -
                           // mapped to 0x10
...
/** Reserve the virtual address space overlapping with the physical address map
 of PULP as well as
 *  the contiguous L3 memory using mmap() syscalls with MAP_FIXED and
 MAP_ANONYMOUS.
 ...
 *
  \param    pulp pointer to the PulpDev structure
 */
HeroSubDev_t *hero_dev_get_l3_mem(HeroDev *dev);
 ...
 *
  \param    pulp pointer to the PulpDev structure
 */
HeroSubDev_t *hero_dev_get_l2_mem(HeroDev *dev);
 ...
 *
  \param    pulp pointer to the PulpDev structure
 */
HeroSubDev_t *hero_dev_get_clusters(HeroDev *dev);
...
/** Request a RAB remapping using one or multiple TLB entries.
  ...
  \param    prot       protection flags, one bit each for write, read, and
  enable
...
/** Request striped RAB remappings
  ...
  \param    pulp       pointer to the PulpDev structure
  \param    task       pointer to the TaskDesc structure
  \param    pass_type  pointer to array marking the elements to pass by
  reference
 ...
 *  The PULP driver function called by this function basically does two things:
 First, it sets up
 *  RAB so that the page table hierarchy can be accessed from PULP.  For this,
 slices either for
 *  the first-level page table or for all second-level page tables are
 configured in RAB
 *  (definable, see parameter below). Second, the driver function disables
 handling of RAB misses
 *  in the Kernel driver itself and prohibits the driver to write to those
 slices that are now
 *  managed by the SoC.

  \param    pulp                    Pointer to the PulpDev struct.
  \param    static_2nd_lvl_slices   If 0, the driver sets up a single RAB slice
 for the first level
                                    of the page table; if 1, the driver sets up
 RAB slices for all
                                    valid second-level page tables.  The latter
 is not supported by
                                    all architectures.  If unsupported, the
 driver will fall back
                                    to the former behavior and emit a warning.

  \return  0 on success; negative value with an errno on errors. See the
 documentation of
 ...
 *  The PULP driver function called by this function frees and deconfigures all
 slices used to map
 *  the initial level of the page table and hands the slices that were reserved
 to be managed by
...
/** Enable handling of RAB misses by the host.
  ...
  \param    pulp       pointer to the PulpDev structure
  \param    use_acp    coherency setup, 0: non-coherent mappings, 1: coherent
  mappings
 ...
 *  This function reads the content of the RAB AX Logger from kernel space,
 sorts the entries
 *  according the timestamp, and writes the data into the file
 `rab_ax_log_%Y-%m-%d_%H-%M-%S.txt`
 *  (see `man date` for the exact meaning of the format specifiers).
 *
 *  This function must be called before freeing the RAB, otherwise the RAB free
 will already empty
...
/** Offload a new task to PULP, set up static RAB entries and pass descriptors
  to PULP.
...
/** Finish a task offload, clear static RAB and get back descriptors passed by
  value.
...
/** Find out which shared data elements to pass by reference and if yes, with
 ...
 *  Try to reorder the shared data elements to minimize the number of entries
 used.
...
/** Copy raw data elements from virtual to contiguous L3 memory and fill pointer
 values in data
 *  descriptor.
