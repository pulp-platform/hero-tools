...
#define __hero_64_define_load_noblock(bits)                                                                            \
    inline static int hero_load_uint##bits##_noblock(const uint64_t addr, __device uint##bits##_t *const val)          \
    {                                                                                                                  \
        __hero_64_noblock_pre(uint##bits##_t) uint##bits##_t reg;                                                      \
        __asm__ volatile(                                                                                              \
            __hero_64_disable_mirq_asm                                                                                 \
            "\n\t"                 /*"csrw 0xbc0, %[upper]\n\t"  set upper 32 address bits in MSEG CSR */              \
            "csrw 0xbc0, zero\n\t" /* TODO: for now, set upper 32 address bits in MSEG CSR to zero to support 32-bit   \
                                      addresses only*/                                                                 \
...
#define __hero_64_define_store_noblock(bits)                                                                           \
    inline static int hero_store_uint##bits##_noblock(const uint64_t addr, const uint##bits##_t val)                   \
    {                                                                                                                  \
        __hero_64_noblock_pre(uint##bits##_t) __asm__ volatile(                                                        \
            __hero_64_disable_mirq_asm                                                                                 \
            "\n\t"                 /*"csrw 0xbc0, %[upper]\n\t"  set upper 32 address bits in MSEG CSR */              \
            "csrw 0xbc0, zero\n\t" /* TODO: for now, set upper 32 address bits in MSEG CSR to zero to support 32-bit   \
                                      addresses only*/                                                                 \
            ...
            __hero_64_restore_mstatus_asm                                                                              \
            : [mstatus] "+&r"(mstatus)                                                                                 \
            : [upper] "r"(upper), [val] "r"(val), [lower] "r"(lower)                                                   \
            : "memory");                                                                                               \
        __hero_64_noblock_post                                                                                         \
    }

#define __hero_64_check_mem_access // TODO: handle misses
...
#define __hero_64_define_load(size)                                                                                    \
    ...
    {                                                                                                                  \
        ...
        __hero_64_check_mem_access return val;                                                                         \
    }
...
#define __hero_64_define(size)                                                                                         \
    __hero_64_define_load_noblock(size) __hero_64_define_load(size) __hero_64_define_store_noblock(size)               \
        __hero_64_define_store(size)

// FIXME: investigate error when this is put at start of the file
#pragma omp declare target
__hero_64_define(32) __hero_64_define(16) __hero_64_define(8)
